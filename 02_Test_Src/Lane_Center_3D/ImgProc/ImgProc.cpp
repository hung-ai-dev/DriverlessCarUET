...
void ImgProc3D::convertTo_Point3fMap(cv::cuda::GpuMat & depth, const ImgProc3D::Intr camInfo, cv::cuda::GpuMat & xyzMat)
{
	...
	caller_convertTo_Point3fMap(depth, camInfo, xyzMat, sz.width, sz.height);
	//cudaGetLastError();
}

void ImgProc3D::genPlane2DMap(cv::cuda::GpuMat & xyzMat, cv::cuda::GpuMat & rgbMat, cv::Vec4f planeModel, cv::cuda::GpuMat & laneMap)
{
	...
	caller_GenGridMap2D(xyzMat, rgbMat, _model, _pOrg, _e_1, _e_2, laneMap, sz.width, sz.height);
}
...
cv::Vec4f fast_PlaneDetect(const cv::Mat & depthMat, cv::Mat & rgbMat, const ImgProc3D::Intr & camInfo)
{
	...
	cv::Point2i invalid_loc;
	cv::Point2i p1_loc = getRandomSample(depthMat, cv::Point2i(imgW / 2, imgH - 60), rng, cv::Point2i(50, 30));
	cv::Point2i p2_loc = getRandomSample(depthMat, cv::Point2i(imgW / 2 - 150, imgH / 2), rng, cv::Point2i(50, 20));
	cv::Point2i p3_loc = getRandomSample(depthMat, cv::Point2i(imgW / 2 + 150, imgH / 2), rng, cv::Point2i(50, 20));
